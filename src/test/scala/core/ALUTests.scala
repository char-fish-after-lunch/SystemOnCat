...
class ALUTester(alu: => ALU) extends BasicTester with TestUtils {
    ...
    val slt  = VecInit((rs1 zip rs2) map { case (a, b) => (if (a.toInt < b.toInt) 1 else 0).U(32.W) })
    val sge  = VecInit((rs1 zip rs2) map { case (a, b) => (if (b.toInt < a.toInt) 1 else 0).U(32.W) })
    val sltu = VecInit((rs1 zip rs2) map { case (a, b) => (if (a < b) 1 else 0).U(32.W) })
    val sgeu = VecInit((rs1 zip rs2) map { case (a, b) => (if (b < a) 1 else 0).U(32.W) })
    val seq = VecInit((rs1 zip rs2) map { case (a, b) => (if (a == b) 1 else 0).U(32.W) })
    val sne = VecInit((rs1 zip rs2) map { case (a, b) => (if (a == b) 0 else 1).U(32.W) })
    ...
    testalu.io.in1 := VecInit(rs1 map (_.S))(cntr)
    testalu.io.in2 := VecInit(rs2 map (_.S))(cntr)

    when(done) { stop(); stop() } // from VendingMachine example...
    printf("Counter: %d, OP: 0x%x, A: 0x%x, B: 0x%x, OUT: 0x%x ?= 0x%x\n",
            cntr, testalu.io.fn, testalu.io.in1, testalu.io.in2, testalu.io.out, out) 
    assert(testalu.io.out === out)
}
