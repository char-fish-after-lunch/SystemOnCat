...
class IFetch extends Module {
    ...
    io.bus.req.ren := (!pc_invalid) && io.core.en

    val pc_reg_invalid = RegInit(Bool(), false.B)
    val pc_reg_en = RegInit(Bool(), false.B)

    pc_reg_invalid := pc_invalid
    pc_reg_en := io.core.en

    io.core.inst := Mux(pc_reg_invalid || !pc_reg_en, NOP, io.bus.res.data_rd)
    io.core.locked := Mux(pc_reg_invalid, false.B, io.bus.res.locked && pc_reg_en)
    io.core.pending := Mux(pc_reg_invalid, false.B, io.pending && pc_reg_en)
