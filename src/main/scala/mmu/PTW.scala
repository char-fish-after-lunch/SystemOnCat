...
class PTW extends Module{
	...
	//Level 2 PTE Check
	val pte_read_valid = (temp_pte.R)  | (mm_cmd === MemoryConsts.PC)
	val pte_write_valid = (temp_pte.W) | (mm_cmd =/= MemoryConsts.Store)
	val pte_exe_valid = (temp_pte.X)   | (mm_cmd =/= MemoryConsts.PC)

	val pte_2_valid = (state =/= s_wait2) | ((temp_pte.V) & pte_read_valid & pte_write_valid & pte_exe_valid & (temp_pte.U | machine_mode))
	//Page Fault Signal
